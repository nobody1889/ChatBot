...
class UserRepository:
    ...
    async def create(self, user: UserCreate) -> User:
        ...
        await self.db.add(user_db)
        await self.db.commit()
        await self.db.refresh(user_db)
        
        return user_db

    async def update(self, user: UserUpdate, data: UserUpdate) -> User:
    ...
    async def update_or_create(self, user: UserCreate) -> User:
        exis = await self.get_by_user_id(user.user_id)
        
        if exis:
            return await self.update(exis, user)
